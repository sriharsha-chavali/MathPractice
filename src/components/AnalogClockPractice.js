...
import AnalogClock from "./AnalogClock"; // The SVG-based clock from previous answer
...
export default function AnalogClockPractice({ QUESTIONS_PER_SET = 12 }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const correct =
      clock.hour === Number(userHour) &&
      clock.minute === Number(userMinute);
    setFeedback(
      correct
        ? "🎉 Correct!"
        : `❌ Oops! It was ${clock.hour}:${clock.minute
            .toString()
            .padStart(2, "0")}`
    );
    setScore((s) => ({
    ...
    return (
      <div style={{ textAlign: "center", marginTop: "2rem" }}>
        ...
        <div style={{ fontSize: "1.5rem", margin: "1rem" }}>
          You got <span style={{ color: "green" }}>{score.right}</span> right
          and <span style={{ color: "red" }}>{score.wrong}</span> wrong out of{" "}
  ...
  return (
    <div style={{ textAlign: "center", marginTop: "2rem" }}>
      <h2>Analog Clocks Practice</h2>
      <div>
        Question: {questionNumber} / {QUESTIONS_PER_SET}
      </div>
      <div>
        Correct: <span style={{ color: "green" }}>{score.right}</span> | Wrong:{" "}
        <span style={{ color: "red" }}>{score.wrong}</span>
      </div>
      <div style={{ margin: "2rem" }}>
        <AnalogClock hour={clock.hour} minute={clock.minute} size={140} />
      </div>
